    ...
    public class Extractor
    {
        ...
        public List<String> ExtractMethods()
        {
            ...
            IEnumerable<MethodDeclarationSyntax> methods = tree.GetRoot()
                                                            .DescendantNodesAndSelf().OfType<MethodDeclarationSyntax>();

            List<String> results = new List<string>();

            foreach (var method in methods)
            {
                //Exclude Interface method
                if (method.Kind() != SyntaxKind.InterfaceDeclaration)
                {
                    if (method.Identifier != null)
                    {
                        String methodName = Utilities.SplitToSubtokens(method.Identifier.ValueText).ToString();

                        var processmethod = RemoveComments(method);
                        processmethod = RemoveEndoflines(processmethod);
                        processmethod = RemoveAttributes(processmethod);
                        var result = processmethod.ToString();
                        if (!string.IsNullOrEmpty(method.Identifier.ValueText))
                        {
                            result = processmethod.ToString().Replace(method.Identifier.ValueText, String.Empty);
                        }
                        result = Regex.Replace(result.ToString(), @"\n", String.Empty);
                        result = Regex.Replace(result.ToString(), @".", " . ");
                        result = Regex.Replace(result.ToString(), @",", string.Empty);
                        result = Regex.Replace(result.ToString(), @"|", string.Empty);
                        result = result.Replace("MethodDeclarationSyntax MethodDeclaration", string.Empty).Trim();
                        results.Add(method.Identifier.ValueText + "|" + result);
                    }
                }
            }
            return results;
        ...
        private static SyntaxNode RemoveComments(SyntaxNode node)
        {
        ...
        private static SyntaxNode RemoveEndoflines(SyntaxNode node)
        {
            IEnumerable<SyntaxTrivia> EOLTrivia = from t in node.DescendantTrivia()
                                                        where t.Kind() == SyntaxKind.EndOfLineTrivia
                                                        select t;
            SyntaxNode newRoot = node.ReplaceTrivia(EOLTrivia, (t1, t2) => default(SyntaxTrivia));
            return newRoot;
        }
        ...
        private static SyntaxNode RemoveAttributes(SyntaxNode node)
        {
            IEnumerable<SyntaxTrivia> attributeTrivia = from t in node.DescendantTrivia()
                                                      where t.Kind() == SyntaxKind.Attribute
                                                      select t;
            SyntaxNode newRoot = node.ReplaceTrivia(attributeTrivia, (t1, t2) => default(SyntaxTrivia));
            return newRoot;
        }
